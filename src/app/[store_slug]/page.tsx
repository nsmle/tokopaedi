...
import { getStoreBySlug, type ProductWithRelations } from "@src/lib/data";
import Link from "next/link";

export default async function Page({ params }: { params: Promise<{ store_slug: string }> }) {
  ...
  return (
    <div className="flex w-full flex-col justify-center">
      <div className="flex w-full flex-col items-center rounded-md bg-white px-4 py-2.5 shadow-2xl/10 transition-all duration-300 hover:shadow-2xl/30 sm:flex-row md:px-6 md:py-4">
        <div className="flex w-full flex-col">
          <p className="font-sans text-[16px] font-semibold text-wrap wrap-anywhere whitespace-pre-line text-gray-700/90">
            {store?.name} <span className="ml-0.5 text-xs text-gray-500/80">@{store?.slug}</span>
          </p>
          <p className="font-sans text-sm font-medium text-wrap wrap-anywhere whitespace-pre-line text-gray-600">{store?.city}</p>
        </div>
        ...
        >
          Detail Toko
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" className="h-4 w-4 text-gray-500/80" fill="currentColor">
      ...
      <ProductList initialProducts={(store?.products || []) as ProductWithRelations[]} storeSlug={store_slug} />
    </div>
  );
}
